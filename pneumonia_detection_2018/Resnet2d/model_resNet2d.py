...
class ResNet2dModule(object):
    ...
    def __init__(self, image_height, image_width, channels=1, n_class=2, costname="cross_entropy",
        ...
        if costname == "focal_loss":
            ...
            self.cost = self.__get_cost(costname, self.Y_gt, self.Y_pred_logits)
    ...
    def __get_cost(self, cost_name, Y_gt, Y_pred):
        if cost_name == "cross_entropy":
            ...
            loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=Y_gt, logits=Y_pred), axis=0)
            cost = tf.reduce_mean(weight_loss * loss)
            return cost
    ...
    def prediction(self, test_images):
        ...
        predictvaluetmp, predict_probvaluetmp = self.sess.run([self.predict, self.Y_pred],
                                                              feed_dict={self.X: test_images,
                                                                         ...
                                                                         self.drop: 1,
                                                                         self.phase: 1})
        predictvalue, predict_probvalue = predictvaluetmp[0], predict_probvaluetmp[0][1]
