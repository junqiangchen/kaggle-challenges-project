...
def save_file2csvclassify(file_dir, file_name):
    ...
    dirs = file_name_path(file_dir, dir=True, file=False)
    for i in range(len(dirs)):
        file_image_dir = file_dir + "/" + str(dirs[i])
        file_paths = file_name_path(file_image_dir, dir=False, file=True)
        for index in range(len(file_paths)):
            out_file_image_path = file_image_dir + "/" + file_paths[index]
            out.writelines(str(dirs[i]) + "," + out_file_image_path + "\n")


if __name__ == '__main__':
    # save_file2csv(r"E:\MedicalData\TNSCUI2020\segmentation\augtrain", "segmeatationaugdata.csv")
    save_file2csvclassify(r"E:\MedicalData\kagglechallenge\Diabetic_Retinopathy\datdaaug", "classifyaugdata.csv")
    print('success')
