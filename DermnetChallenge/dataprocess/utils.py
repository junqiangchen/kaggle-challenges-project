...
def save_file2csvclassifyv2(file_dir, file_name):
    out = open(file_name, 'w')
    out.writelines("label,Image" + "\n")
    dirs = file_name_path(file_dir)
    for dir in range(len(dirs)):
        file_image_dir = file_dir + "/" + dirs[dir]
        file_paths = file_name_path(file_image_dir, dir=False, file=True)
        for index in range(len(file_paths)):
            out_file_image_path = file_image_dir + "/" + file_paths[index]
            out.writelines(str(dirs[dir]) + "," + out_file_image_path + "\n")


if __name__ == '__main__':
    # save_file2csv(r"E:\MedicalData\TNSCUI2020\segmentation\augtrain", "segmeatationaugdata.csv")
    # save_file2csvclassify(r"E:\MedicalData\TNSCUI2020\classifiy\augtrain", "classifyaugdata.csv")
    save_file2csvclassifyv2(r"E:\MedicalData\kagglechallenge\Dermnet\train_aug", "trainaugdata.csv")
    print('success')
